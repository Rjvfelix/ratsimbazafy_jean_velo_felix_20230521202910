...
@Bean
    ...
    public InMemoryUserDetailsManager inMemoryUserDetailsManager(){
        return new InMemoryUserDetailsManager(
                User.withUsername("user").password("{noop}1234").authorities("ADMIN").build(),
                User.withUsername("anywr-group-recruitement").password("{noop}1234").authorities("ADMIN").build()
        );
    }
